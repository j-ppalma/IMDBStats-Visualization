...
function(input, output) {
  #################DATAFRAMA VIS1###################
   ...
   datafilter<-reactive({
     df1<-df[str_detect(df$genres,input$genre),]
     df1 <- df1%>%
  ...
  numVotes<-reactive({
    votes<-df1%>% filter(title==global$selectedBar)%>%select( imdb_votes)
    votes[1,"imdb_votes"]
  })
   ...
   numScore<-reactive({
     score<-df1%>% filter(title==global$selectedBar)%>%select(imdb_score)
     score[1,"imdb_score"]
   })
  ...
  #################Dataframe 2###################
   df2<-titles %>% select(-id,-description,-imdb_id)
   
  #################Visualization 2################
   observeEvent(input$mytable_rows_selected, {
      desc<-df1%>%filter(grepl(df2[input$mytable_rows_selected,1],title,fixed = TRUE))%>%select(description)
